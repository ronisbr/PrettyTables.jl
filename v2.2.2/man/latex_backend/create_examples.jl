...
run(`cp example.tex format_double.tex`)
...
run(`pdflatex format_double.tex`)
run(`pdflatex format_double.tex`)
run(`convert -density 150 -trim -strip format_double.pdf format_double.png`)
run(`rm format_double.tex`)
run(`rm format_double.pdf`)
...
hl_e = LatexHighlighter(
    (data, i, j) -> (i == 10),
    ["cellcolor{black}", "color{white}", "textbf"]
)

run(`cp example.tex latex_highlighter.tex`)

include_pt_in_file(
    "latex_highlighter.tex",
    "Table 1",
    data;
    backend      = Val(:latex),
    backup_file  = false,
