...
- If `filename === nothing`
    - If `sheet === XLSX.worksheet`: Return `nothing`. The worksheet specified is updated in
        place.
    - If `sheet !== XLSX.worksheet`: Return an in-memory `XLSX.XLSXFile` object.
...
!!! note
    ...
    The returned XLSX.XLSXFile can be saved using `XLSX.writexlsx` or `XLSX.savexlsx`.
...
- `sheet::Union{String, XLSX.Worksheet}`: If `sheet` is a `String`, it specifies the name of
    ...
    the given name doesn't exist, it will be created. If `sheet` is an `XLSX.Worksheet`,
    this worksheet will be updated in place by the addition of the pretty table and
...
- `anchor_cell::String`: Defines the top-left cell of the table, allowing placement anywhere
    ...
    the specified sheet.
    (**Default**: `"A1"`).
- `excel_formatters::Vector{ExcelFormatter}`: Excel-specific format (numFmt) definitions to
    appy to the table.
- `highlighters::Vector{ExcelHighlighter}`: Excel-specific highlighters to apply to the
      table.
- `table_format::ExcelTableFormat`: Defines the table borders to be used in each section of
    the table.
