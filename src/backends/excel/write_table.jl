...
function _write_excel_table!(sheet, table_data::TableData;
    ...
    if !isempty(table_data.title)
        max_row_height[current_row] = _excel_write_title!(sheet, table_data, style, footnote_refs, current_row, num_cols, anchor_row_offset, anchor_col_offset, col_offset)
        current_row += 1
     end
    
    # Write subtitle if present
    if !isempty(table_data.subtitle)
        max_row_height[current_row] = _excel_write_subtitle!(sheet, table_data, style, footnote_refs, current_row, num_cols, anchor_row_offset, anchor_col_offset, col_offset)
        current_row += 1
     end
    ...
    if table_data.show_row_number_column
        max_row_height[current_row] = _excel_write_row_number_column!(sheet, table_data, table_format, style, max_row_height[current_row], max_col_length, anchor_row_offset, anchor_col_offset, current_row)
    end
        ...
        for (label_row_idx, label_row) in enumerate(table_data.column_labels)
            ...
            if table_data.row_labels !== nothing
                if label_row_idx == 1 && !isempty(table_data.stubhead_label)
                    max_row_height[current_row] = _excel_write_stubhead_label!(sheet, table_data, style, col_offset, max_row_height[current_row], max_col_length, anchor_row_offset, anchor_col_offset, current_row)
                end
            end
            ...
            while j <= length(label_row)
                j, max_row_height[current_row] = _excel_write_column_labels!(sheet, table_data, table_format, style, footnote_refs, merge_map, label_row, label_row_idx, j, num_cols, col_offset, max_row_height[current_row], max_col_length, anchor_row_offset, anchor_col_offset, current_row)
            end

            setRowHeight(sheet, current_row + anchor_row_offset; height = max_row_height[current_row])
    ...
    for i in 1:num_rows
        ...
        if haskey(row_group_map, i)
            max_row_height[current_row] = _excel_write_group_row!(sheet, table_data, table_format, style, footnote_refs, row_group_map, i, num_cols, col_offset, max_row_height[current_row], anchor_row_offset, anchor_col_offset, current_row)
            current_row += 1
            max_row_height[current_row] = 0.0
        end
        ...
        if table_data.show_row_number_column
            max_row_height[current_row] = _excel_write_row_number!(sheet, table_data, table_format, style, i, max_row_height[current_row], anchor_row_offset, anchor_col_offset, current_row)
        end
        ...
        if table_data.row_labels !== nothing && i <= length(table_data.row_labels)
            max_row_height[current_row] = _excel_write_row_label(sheet, table_data, style, footnote_refs, i, row_label_col, max_row_height[current_row], max_col_length, anchor_row_offset, anchor_col_offset, current_row)
        end
       ...
       # Do before writing cell content and highlighting
        ...
        # Write data cells
        for j in 1:num_cols
            max_row_height[current_row] = _excel_write_cell!(sheet, table_data, table_format, style, highlighters, excel_formatters, i, j, num_cols, col_offset, footnote_refs, max_row_height[current_row], max_col_length, anchor_row_offset, anchor_col_offset, current_row)
        end
    ...
    if table_data.summary_rows !== nothing
        for (idx, summary_row_func) in enumerate(table_data.summary_rows)
            ...
            _excel_unempty_row(sheet, current_row + anchor_row_offset, 1 + anchor_col_offset:num_cols+col_offset + anchor_col_offset) # ensure these cells aren't empty before merging
            max_row_height[current_row] = _excel_write_summary_row(sheet, table_data, table_format, style, idx, summary_row_func, excel_formatters, num_cols, col_offset, footnote_refs, max_row_height[current_row], max_col_length, anchor_row_offset, anchor_col_offset, current_row)
    ...
    if table_data.footnotes !== nothing && !isempty(table_data.footnotes)
        _excel_unempty_row(sheet, current_row + anchor_row_offset, 1 + anchor_col_offset:num_cols+col_offset + anchor_col_offset) # ensure these cells aren't empty before merging
        current_row = _excel_write_footnotes!(sheet, table_data, table_format, style, current_row, num_cols, max_row_height, anchor_row_offset, anchor_col_offset, col_offset)
     end
    ...
    if !isempty(table_data.source_notes)
        ...
        _excel_unempty_row(sheet, current_row + anchor_row_offset, 1 + anchor_col_offset:num_cols+col_offset + anchor_col_offset) # ensure these cells aren't empty before merging
        max_row_height[current_row] = _write_excel_sourcenotes!(sheet, table_data, style, current_row, num_cols, max_row_height[current_row], anchor_row_offset, anchor_col_offset, col_offset)
        current_row += 1
    end
                ...
                for highlighter in highlighters
                    ...
                    if !isnothing(atts)
                        ...
                        if !isempty(font_atts)
                            ...
                            setFont(sheet, start_table_row + i, j + col_offset + anchor_col_offset; font_atts...)
                        end
                        if !isempty(fill_atts)
                            setFill(sheet, start_table_row + i, j + col_offset + anchor_col_offset; fill_atts...)
                        end
                        if !isempty(border_atts)
                            setBorder(sheet, start_table_row + i, j + col_offset + anchor_col_offset; allsides = [border_atts...])
                        end
                        if !isempty(border_atts)
                            setBorder(sheet, start_table_row + i, j + col_offset + anchor_col_offset; allsides = [border_atts...])
                        end
    ...
    for (row, height) in max_row_height
        setRowHeight(sheet, row + anchor_row_offset; height = height)
    end
